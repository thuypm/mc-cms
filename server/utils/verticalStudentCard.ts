...
async function drawAvatarImage(
  ...
  try {
    const avatarPath = path.join(process.cwd(), "public", "images", fileName);
    const avatarImg = await loadImage(avatarPath);
    const { width, height } = avatarImg;
    ...
    if (actualRatio < targetRatio) {
      ...
      // Ảnh quá rộng → cắt 2 bên (nếu bạn muốn, không cần ở đây)
      sw = height * targetRatio;
      sx = (width - sw) / 2;
    }

    // Vẽ phần đã cắt vào đúng khung 3x4
    ctx.drawImage(
      avatarImg,
      sx,
      sy,
...
export const generateStudentCard = async (
  ...
  try {
    ...
    await drawAvatarImage(ctx, student.image, config.image);
    // QR code
