...
def find_darkest_circle_per_column(roi_gray, rows=10, cols=3, binary_thresh=150, threshold_diff=3.0):
    """
...
def draw_debug_circles_on_roi(roi_color, debug_circles):
    """
    Vẽ các hình tròn được chọn lên ảnh ROI để debug.
    ...
    for col, row, x, y, r in debug_circles:
        ...
        cv2.putText(debug_img, f"C{col}:R{row}", (x - 10, y + 5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    return debug_img

def processKey(img, roi= configPaper["KEY_BOUND"], binary_thresh=150, debug=False):
    if img is None:
...
if __name__ == "__main__":
    ...
    # ROI theo tọa độ của ảnh đã căn chỉnh (ví dụ)
    roi_coordinates = (897, 102, 991, 376)

    # Chỉnh giá trị binary_thresh nếu cần
    digits, debug_img = process_matrix_image(image_path, roi_coordinates, binary_thresh=150)

    print("Kết quả nhận dạng theo cột (mảng 2 chiều):")
    for i, col_digits in enumerate(digits):
        print(f"Cột {i+1}: {col_digits}")
