...
def find_darkest_circle_per_row(roi_gray, rows=4, cols=2, binary_thresh=150, threshold_diff=10.0, mean_val_thresh=200):
    """
...
def draw_debug_circles_on_roi(roi_color, debug_circles):
    ...
    col_to_letter = ['D', "S"]  # Cột 0 -> A, cột 1 -> B, v.v.
...
def process_matrix_image(img, roi, binary_thresh=150, debug = False):
    if img is None:
...
def processPartTwo(img):
    mapDigist = []
    listRois = configPaper["PARTTWO_BOUND"]

    for rois in listRois:
        digits = process_matrix_image(img, rois, binary_thresh=150)
